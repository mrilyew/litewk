window.page_class = new class {
    async render_page() {
        ...
        window.main_classes['wall'].setParams('wall.get', wall_params, window.main_url.searchParams.get('wall_invert') == 'yes')
        ...
        if(section != 'search') {
            ...
            if(window.main_url.searchParams.get('wall_query')) {
                $(`input[type='query']`)[0].value = window.main_url.searchParams.get('wall_query')
                await window.main_classes['wall'].search(window.main_url.searchParams.get('wall_query'))
            }
        }

        try {
            $('.wall_wrapper span')[0].innerHTML = _('counters.posts_on_wall_count', window.main_classes['wall'].objects.count)
            $('.wall_wrapper_upper_panel')[0].insertAdjacentHTML('beforeend', window.templates.paginator(window.main_classes['wall'].objects.pagesCount, (Number(window.main_url.searchParams.get('page') ?? 1))))
        } catch(e) {}
    }
}
