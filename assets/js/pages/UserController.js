window.controllers['UserController'] = (function() {
    return {
        UserPage: async function() {
            ...
            u('.user_page_grid').on('click', '#_setReport', (e) => {
                const msg = new MessageBox(_('common.report'), window.templates._report_user_block(user), [_('messagebox.cancel'), _('messagebox.send')], [() => {
                    msg.close()
                }, async () => {
            ...
            u('.user_page_grid').on('click', '#_friendStatusChange', async (e) => {
                ...
                u('.entity_actions').addClass('stopped')
                ...
                u('.entity_actions').removeClass('stopped')
                u('.page_content .entity_actions').html(window.templates._user_page_buttons(user))
                ...
                try {
                    ...
                    switch(bl_status) {
                        ...
                        case 1:
                            ...
                            user.info.blacklisted_by_me = 0
        
                            break
                    }

                    user.cacheEntity(true)
                } catch(e) {}
                u('.entity_actions').removeClass('stopped')
            })
        },
        UserPageSkeleton: function() {
