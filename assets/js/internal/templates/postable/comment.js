...
window.templates.comment = (object) => {
    ...
    const shorted_text = Utils.get_short_text(object.info.text, 10, 500, true)
            ...
            <div class='comment_block main_info_block' data-type='comment' data-postid='${object.getId()}'>
                <div class='comment_author'>
                    <div class='comment_avaname avatar'>
                        <a ${owner.isOnline() ? `class='onliner'` : ''} href='${owner ? owner.getUrl() : ''}'>
                            <img loading='lazy' class='outliner' src='${owner ? owner.getAvatar(true) : ''}'>
                        </a>
                    </div>
                </div>
                <div class='comment_info'>
                    ...
                    <div class='comment_bottom'>
                        <span>
                            <a href='javascript:void(0)'>${object.getDate()}</a>
                        </span>
                        <div class='post_actions_no_frame'>
                            <div class='_like_another like like_not_post ${object.isLiked() ? 'activated' : '' }'>
                                <svg class='like_icon_hearted' viewBox="0 0 17 16.22"><g><path d="M12.43,1A4.67,4.67,0,0,0,9,2.48,4.67,4.67,0,0,0,5.57,1,4.44,4.44,0,0,0,1,5.29C1,7.65,9,16,9,16s8-8.35,8-10.71A4.44,4.44,0,0,0,12.43,1Z" transform="translate(-0.5 -0.5)"/></g></svg>
                                ${object.hasLikes() ? `<span class='likes_handler'>${object.getLikes()}</span>` : ''}
                            </div>
                        </div>
                    </div>
                </div>
