...
class Newsfeed extends ClassicListView {
    ...
    async nextPage() {
        ...
        let error = (msg = objects_data.error_msg) => {
            this.getInsertNode().insertAdjacentHTML('beforeend', `
                <div class='bordered_block'>${_('errors.error_getting_news', msg)}</div>
            `)
        }
        ...
        try {
            if(window.use_execute) {
                ...
                if(!this.method_params.start_from) {
                    ...
                    objects_data.response.lists = lists
                }

                objects_data.response.news = news
            }
        } catch(e) {
            console.error(e)
            error(e.message)
        ...
        this.updateReactions(objects_data.response.news.reaction_sets)
        objects_data.response.news.items.forEach(obj => {
            ...
            ob_j.hydrate(obj, objects_data.response.news.profiles, objects_data.response.news.groups, this.objects.reaction_sets)
