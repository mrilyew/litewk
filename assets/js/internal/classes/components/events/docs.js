$(document).on('click', '.document_block .document_block_actions .add_doc_button', async (e) => {
    e.preventDefault()
    ...
    if(target.dataset.type == 'add') {
        ...
        if(!res.error) {
            ...
            target.setAttribute('data-secondid', res.response)
...
$(document).on('click', '.delete_doc_button', async (e) => {
    e.preventDefault()
...
$(document).on('click', '.edit_doc_button_wrap', async (e) => {
    e.preventDefault()
    ...
    let info = JSON.parse(target.dataset.inform)
    let doc = new Doc
    doc.hydrate(info)
    ...
    `, [_('messagebox.cancel'), _('messagebox.save')], [() => {
        ...
        let name  = $(`#doc_edit_box input[name='title']`)[0].value
        let normal_tags = $(`#doc_edit_box input[name='tags']`)[0].value
        let type = $(`input[type='radio'][name='sn_folder']:checked`).val()
        let arrayed_tags = []
        let tags = ''
...
$(document).on('click', '#__tagadder', (e) => {
